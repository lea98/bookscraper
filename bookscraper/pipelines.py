...
class BookscraperPipeline:  # everytime we yield items it goes to this file
    ...
    def store_db(self, item):
        ...
        if not exists_in:
            ...
            for auth in item['author']:
                auth_is_there_list = self.db.execute(f"""select id from authors where name = '{auth.replace("'","''")}';""").fetchone()
                if not auth_is_there_list:
                    auth_id_num = self.db.execute(f"""insert into authors values (DEFAULT,'{auth.replace("'","''")}') RETURNING id;""").first()[0]
                else:
    ...
    def check_if_exists_in_table(self,item):
        ...
        for book_id in list(all_books_with_that_name):
            book_authors_match = self.db.execute(f"""select author_id from book_authors where book_id = {book_id[0]};""")
