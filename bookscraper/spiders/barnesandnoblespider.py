...
class BarnesSpider(scrapy.Spider):
        ...
        for index, product in enumerate(response.xpath("//div[@class='row topX-row']"), start=1):
            try:
                yield {
                    ...
                    'author': [s.strip() for s in product.xpath(
                        f"(//div[@class='product-info-view'])[{index}]//div[contains(@class,'product-shelf-author contributors')]/descendant::a//text()").extract()],
                    'price': product.xpath(".//a[@class=' current link']/text()").get().split(';jsessionid')[0],
                    'link': self.form_link('https://www.barnesandnoble.com' +
                                           product.xpath(".//h3[@class='product-info-title']//a").attrib['href']),
                    'page': 1,
    ...
    def form_link(link):
        if ';jsessionid' in link:
            return \
            (link.split(';jsessionid')[0] + '?ean' + link.split('?ean')[1]).split('https://www.barnesandnoble.com/')[1]
        else:
