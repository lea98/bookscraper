        ...
        for product in response.xpath("//div[@class='item-inner']"):
            try:
                yield {
                    ...
                    'price': product.xpath(
                        ".//div[@class='product-description']//span[@class='price']/text()").get().replace(u'\xa0',
                                                                                                           u' ').replace(
                        'HRK', 'kn'),
                    'link': product.xpath(".//span[@class='cover_image']//a").attrib['href'].split(
                        'https://www.ljevak.hr/')[1],
                    'page': 4,
