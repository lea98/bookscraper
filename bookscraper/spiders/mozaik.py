...
class MozaikSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for index, product in enumerate(
            ...
            try:
                ...
                yield {
                    ...
                    "link": product.xpath(".//div[@class='title']//a")
                    .attrib["href"]
                    .split("https://mozaik-knjiga.hr/")[1],
                    "page": 3,
        ...
        # COMMENT FOR LOCAL TESTING, DON'T FILL PROD DB WITH MUCH DATA
        if NEXT_PAGE_NUM_MOZAIK <= STOP_PAGE:
            yield response.follow(
                f"https://mozaik-knjiga.hr/kategorija/knjige/knjizevnost/page/{NEXT_PAGE_NUM_MOZAIK}/",
                callback=self.parse,
            )
    ...
    def get_authors(product, index):
        authors = [
            s.strip()
            for s in product.xpath(
                f"(//div[@class='books-container']/div[contains(@class,'book')])[{index}]//div[@class='author']//a//text()"
            ).extract()
        ]
        additional_checker = []
        for i in authors:
    ...
    def get_currency(product):
        ...
        if two_prices:
            return (
                product.xpath(".//p[@class='price']//ins/span//text()")
                .get()
                .replace("\xa0", " ")
                + product.xpath(".//p[@class='price']//ins/span/span//text()").get()
            )
        else:
            return (
                product.xpath(".//p[@class='price']/span//text()")
                .get()
                .replace("\xa0", " ")
                + product.xpath(".//p[@class='price']/span/span/text()").get()
            )
    ...
    def get_page_to_top(res):
        ...
        if STOP_PAGE:
            return
        STOP_PAGE = float(
            res.xpath("//a[@aria-label='Last Page']")
            .attrib["href"]
            .split("/page/")[1]
            .split("/")[0]
        )
